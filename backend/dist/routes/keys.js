...
router.post("/", async (req, res) => {
    ...
    const encryptedKey = (0, crypto_1.encrypt)(key);
    const { error } = await supabase_1.supabase.from("user_api_keys").upsert({
        user_id: userId,
        provider,
...
router.post("/:provider/test", async (req, res) => {
    ...
    const { data: keyRow, error: dbError } = await supabase_1.supabase
        .from("user_api_keys")
    ...
    const apiKey = (0, crypto_1.decrypt)(keyRow.encrypted_key);
    try {
...
async function testProviderKey(provider, key) {
    switch (provider) {
        ...
        case "rapidapi": {
            ...
            const res = await fetch("https://instagram120.p.rapidapi.com/api/instagram/user?username=instagram", {
                headers: {
        ...
        case "bedrock": {
            ...
            if (!key || key.length < 20)
                throw new Error("Invalid Bedrock token format");
