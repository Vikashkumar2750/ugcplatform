...
router.post("/enqueue", async (req, res) => {
    ...
    if (workerSecret === process.env.WORKER_SECRET) {
        ...
        const anonClient = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_ANON_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY);
...
router.post("/ai-reply", auth_1.requireAuth, async (req, res) => {
    const { userId } = req;
    const { accountId, recipientId, conversationId, incomingMessage, conversationHistory, businessContext, replyStyle, language, } = req.body;
    ...
    try {
        const result = await (0, ai_reply_1.generateAIReply)({
            userId,
            accountId,
            recipientId,
            conversationId,
...
router.get("/queue-stats", auth_1.requireAuth, async (req, res) => {
    const accountId = req.query.accountId;
    try {
        const stats = await (0, send_queue_1.getQueueStats)(accountId);
...
router.get("/rate-limit-status", auth_1.requireAuth, async (req, res) => {
    const accountId = req.query.accountId;
    if (!accountId) {
        return res.status(400).json({ error: "accountId query param required" });
    }
    try {
        const status = await (0, rate_limiter_1.getRateLimitStatus)(accountId);
...
router.get("/compliance-logs", auth_1.requireAuth, async (req, res) => {
    const { userId } = req;
    const limit = parseInt(req.query.limit) || 50;
    const offset = parseInt(req.query.offset) || 0;
    const decision = req.query.decision; // 'allowed' | 'blocked'
    try {
...
router.get("/reviews", auth_1.requireAuth, async (req, res) => {
    const { userId } = req;
    const limit = parseInt(req.query.limit) || 20;
    try {
        const reviews = await (0, ai_reply_1.getPendingReviews)(userId, limit);
...
router.post("/reviews/:id/approve", auth_1.requireAuth, async (req, res) => {
    const { userId } = req;
    const id = req.params.id;
    const { editedText } = req.body;
    try {
        const result = await (0, ai_reply_1.approveReviewedMessage)(id, userId, editedText);
        if (!result.success) {
        ...
        if (review) {
            ...
            const enqueueResult = await (0, send_queue_1.enqueueMessage)({
...
router.post("/reviews/:id/discard", auth_1.requireAuth, async (req, res) => {
    const { userId } = req;
    const id = req.params.id;
    try {
        const result = await (0, ai_reply_1.discardReviewedMessage)(id, userId);
        return res.json(result);
    }
    catch (err) {
        return res.status(500).json({ error: err.message });
