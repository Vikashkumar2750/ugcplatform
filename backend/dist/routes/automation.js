...
router.post("/schedule", async (req, res) => {
    const { userId } = req;
    const { platform, accountId, caption, mediaUrl, mediaUrls, contentType, scheduledAt, title, } = req.body;
...
router.post("/publish/:id", async (req, res) => {
    ...
    try {
        ...
        if (post.status === "cancelled")
            return res.status(400).json({ error: "Post was cancelled" });
                ...
                if (count === 0) {
                    ...
                    try {
                        ...
                        if (pathParts.length > 1) {
                            ...
                            await supabase_1.supabase.storage.from("post-media").remove([pathToDelete]);
                        }
                    }
                    catch (e) {
        ...
        // Mark as failed
        await supabase_1.supabase.from("scheduled_posts").update({ status: "failed" }).eq("id", id);
        return res.status(500).json({ error: err.message });
    }
});
...
async function publishToInstagram(opts) {
    ...
    if (contentType === "carousel" && mediaUrls?.length) {
        ...
        for (const url of mediaUrls) {
            ...
            if (d.error)
                throw new Error(`IG carousel item: ${d.error.message}`);
            itemIds.push(d.id);
        }
        ...
        if (container.error)
            throw new Error(`IG carousel container: ${container.error.message}`);
        ...
        if (pub.error)
            throw new Error(`IG publish: ${pub.error.message}`);
        return pub.id;
    }
    // Reel
    if (contentType === "reel" && mediaUrl) {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            if (status.status_code === "ERROR")
                throw new Error("IG reel processing failed");
        }
        const pubRes = await fetch(`${base}/${igId}/media_publish`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        const pub = await pubRes.json();
        if (pub.error)
            throw new Error(`IG reel publish: ${pub.error.message}`);
        return pub.id;
    }
    // Photo / Story
    if (mediaUrl) {
        ...
        const container = await containerRes.json();
        if (container.error)
            throw new Error(`IG photo container: ${container.error.message}`);
        const pubRes = await fetch(`${base}/${igId}/media_publish`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ creation_id: container.id, access_token: token }),
        ...
        const pub = await pubRes.json();
        if (pub.error)
            throw new Error(`IG photo publish: ${pub.error.message}`);
        return pub.id;
    }
...
async function publishToFacebook(opts) {
    ...
    if ((contentType === "video" || contentType === "reel") && mediaUrl) {
        ...
        const d = await r.json();
        if (d.error)
            throw new Error(`FB video: ${d.error.message}`);
        return d.id;
    }
    if (mediaUrl) {
        ...
        const d = await r.json();
        if (d.error)
            throw new Error(`FB photo: ${d.error.message}`);
    ...
    const d = await r.json();
    if (d.error)
        throw new Error(`FB post: ${d.error.message}`);
    return d.id;
}
