...
const meta_data_1 = require("../services/meta-data");
...
function validateLLMOutput(data, endpoint) {
    if (!data || typeof data !== "object")
        return { valid: false, warnings: ["Empty or non-object response"] };
...
async function getUserScraperKeys(userId) {
    const { data: keyRows } = await supabase_1.supabase
        .from("user_api_keys")
    ...
    for (const row of keyRows || []) {
        try {
            if (row.provider === "rapidapi")
                userKeys.rapidapi = (0, crypto_1.decrypt)(row.encrypted_key);
            if (row.provider === "apify")
                userKeys.apify = (0, crypto_1.decrypt)(row.encrypted_key);
        }
        catch { }
    }
    return userKeys;
...
async function getRealProfileData(userId, platform, profileUrl) {
    const username = extractUsername(profileUrl, platform);
    // 1. Try Meta Graph API (connected account — most reliable, free)
    if (platform === "instagram") {
        const data = await (0, meta_data_1.fetchConnectedInstagramData)(userId, username);
        if (data)
            return { data, source: "meta_graph_api" };
    }
    if (platform === "facebook") {
        const data = await (0, meta_data_1.fetchConnectedFacebookData)(userId, username);
        if (data)
            return { data, source: "meta_graph_api" };
    }
    ...
    try {
        if (platform === "instagram" && username) {
            const result = await (0, scraper_1.scrapeInstagramProfile)(username, userKeys);
...
router.post("/audit", async (req, res) => {
    ...
    try {
        ...
        if (realData && realData.posts && realData.posts.length > 0) {
            ...
            const intel = (0, intelligence_1.aggregateIntelligence)(intelPosts);
            intelligenceSummary = `
...
Return JSON:
{
  ...
  "topRecommendation": "The single most brutal, actionable change they need to make right now."
        ...
        const { data: saved } = await supabase_1.supabase.from("analysis_results").insert({
            user_id: userId,
            platform,
            result: {
                type: "audit",
...
Return ONLY this JSON array. No markdown, no comments, no extra text.`;
            try {
                const discoverRes = await (0, llm_1.callLLM)({
                    userId,
        ...
        if (enhancedCompetitors.length === 0 && targetCompetitors?.length && platform === "instagram") {
            ...
            const userKeys = await getUserScraperKeys(userId);
            const results = await Promise.allSettled(usernames.map((u) => (0, scraper_1.scrapeCompetitorFull)(u, userKeys)));
...
ADVANCED INTELLIGENCE DATA:
- Dominant Hook Types: ${intel.trend_intelligence.dominant_hook_types.map((h) => `${h.type}(${h.count})`).join(", ")}
- Content Categories: ${intel.trend_intelligence.dominant_content_categories.map((cat) => `${cat.category}(${cat.count})`).join(", ")}
- Top Hashtags: ${intel.trend_intelligence.top_hashtags.map((t) => t.tag).join(", ")}
- CTA Patterns: ${intel.trend_intelligence.cta_patterns.slice(0, 2).map((cta) => `"${cta.hook}"`).join(", ")}
...
Return ONLY this JSON structure:
{
  ...
  "postingStrategy": {
    ...
    "reason": "why this works for the detected audience"
        ...
        const llmResult = await (0, llm_1.callLLM)({ userId, endpoint: "competitors", prompt, systemPrompt });
        const data = extractJSON(llmResult.text) || { raw: llmResult.text };
        // Validate LLM output quality
        const { valid, warnings } = validateLLMOutput(data, "competitors");
        if (!valid)
            console.warn("[competitors] LLM output quality warnings:", warnings);
...
router.post("/trends", async (req, res) => {
    ...
    try {
        ...
        try {
            if (platform === "youtube") {
                trendData = await (0, scraper_1.runApifyActor)("streamers/youtube-scraper", {
...
Return ONLY a valid JSON object. No explanation, no markdown wrappers.`;
        ...
        const { countPlaceholders, stripPlaceholders } = await Promise.resolve().then(() => __importStar(require("../services/llm")));
...
CRITICAL REMINDER: Zero [brackets]. Every word real and ready to record.`;
        ...
        for (let w = 0; w < WEEK_DEFS.length; w++) {
            ...
            for (let batchStart = 0; batchStart < 7; batchStart += batchSize) {
                ...
                const postPromises = batchIndices.map(pi => (0, llm_1.callLLM)({ userId, endpoint: `pipeline_w${wd.week}_p${pi + 1}`, prompt: buildPostPrompt(wd, pi, competitorPostsContext), systemPrompt })
                    .then(r => ({ ok: true, text: r.text, provider: r.provider, model: r.model }))
                ...
                for (let bi = 0; bi < postResults.length; bi++) {
                    ...
                    exports.pipelineProgressMap.set(progressKey, Math.min(95, currentProgress + 12.8));
                    if (!pr.ok) {
                        console.warn(`[pipeline] w${w + 1} p${pi + 1} LLM failed: ${pr.error}`);
                        continue;
                    }
                    let parsed = extractJSON(pr.text);
...
{"contentPillars":[{"pillar":"ACTUAL pillar 1 for ${effectiveNiche}","percentage":30,"examples":["idea1","idea2","idea3"]},{"pillar":"ACTUAL pillar 2","percentage":25,"examples":["idea1","idea2"]},{"pillar":"ACTUAL pillar 3","percentage":20,"examples":["idea1","idea2"]},{"pillar":"ACTUAL pillar 4","percentage":15,"examples":["idea1"]},{"pillar":"ACTUAL pillar 5 - Community/BTS","percentage":10,"examples":["idea1"]}],"batchingStrategy":"How to shoot all 28 posts in 3 days for ${effectiveNiche} creator - equipment, outfit changes, shooting order, location changes","postingSchedule":{"frequency":"7 posts/week (daily posting)","bestDays":["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],"bestTimes":{"Monday":"8:30 AM IST","Tuesday":"12:30 PM IST","Wednesday":"7:00 PM IST","Thursday":"9:00 AM IST","Friday":"6:30 PM IST","Saturday":"10:00 AM IST","Sunday":"11:00 AM IST"},"reason":"Why these specific times work for Indian ${effectiveNiche} audience"},"kpis":{"targetER":"${targetER}%","postingFrequency":"7/week","growthTarget":"Realistic 30-day target for ${effectiveNiche}","savesTarget":"Target save rate per post","shareTarget":"Target shares per week"}}`;
        let metaData = { contentPillars: [], postingSchedule: { bestDays: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"], bestTimes: { "Monday": "8:30 AM IST" } }, kpis: { targetER: targetER + "%", postingFrequency: "7/week", growthTarget: "1000+ followers/month" } };
        try {
            const metaRes = await (0, llm_1.callLLM)({ userId, endpoint: "pipeline_meta", prompt: metaPrompt, systemPrompt });
            const pm = extractJSON(metaRes.text);
            if (pm)
                metaData = pm;
        }
        catch {
            console.warn("[pipeline] Meta call failed");
        }
...
router.post("/save", async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supabase_1.supabase.from("analysis_results").insert({
            user_id: userId,
            platform,
            result: {
                type: "full",
