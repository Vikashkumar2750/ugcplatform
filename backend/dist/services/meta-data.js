...
exports.fetchConnectedInstagramData = fetchConnectedInstagramData;
exports.fetchConnectedFacebookData = fetchConnectedFacebookData;
const supabase_1 = require("../lib/supabase");
// Fetch real Instagram data for a user's connected account
async function fetchConnectedInstagramData(userId, targetUsername) {
    try {
        ...
        // 1. Profile
        const profileRes = await fetch(`${base}/${igId}?fields=id,username,name,biography,followers_count,follows_count,media_count&access_token=${token}`);
        ...
        const mediaRes = await fetch(`${base}/${igId}/media?fields=id,timestamp,media_type,like_count,comments_count,caption,permalink&limit=20&access_token=${token}`);
        const mediaData = await mediaRes.json();
        const rawPosts = mediaData?.data || [];
        // 3. Get insights for top 5 posts (saves + reach)
        const enrichedPosts = await Promise.all(rawPosts.slice(0, 20).map(async (p) => {
            let saves = 0;
            let reach = 0;
            try {
                const insRes = await fetch(`${base}/${p.id}/insights?metric=saved,reach&access_token=${token}`);
                const ins = await insRes.json();
                const find = (name) => ins?.data?.find((m) => m.name === name)?.values?.[0]?.value || 0;
                saves = find("saved");
                reach = find("reach");
            }
            catch { }
...
async function fetchConnectedFacebookData(userId, targetUsername) {
    try {
        const { data: account } = await supabase_1.supabase
            .from("connected_accounts")
            .select("*")
        ...
        const token = account.access_token;
        const base = `https://graph.facebook.com/v21.0`;
        const pageRes = await fetch(`${base}/${pageId}?fields=id,name,username,fan_count,followers_count,biography&access_token=${token}`);
        const page = await pageRes.json();
        if (page.error)
            return null;
        const postsRes = await fetch(`${base}/${pageId}/posts?fields=id,message,created_time,reactions.summary(true),comments.summary(true)&limit=20&access_token=${token}`);
