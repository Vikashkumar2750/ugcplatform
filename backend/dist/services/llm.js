...
exports.stripPlaceholders = stripPlaceholders;
...
async function callLLM(req) {
    ...
    for (const row of keyRows || []) {
        try {
            userKeys[row.provider] = (0, crypto_1.decrypt)(row.encrypted_key);
        }
        catch { }
    }
    ...
    try {
        await supabase_1.supabase.from("api_usage_logs").insert({
            user_id: userId,
...
async function callGeminiModel(apiKey, model, prompt, systemPrompt) {
    const contents = [];
    if (systemPrompt) {
...
function isSkippableGeminiError(msg) {
...
async function callGemini(apiKey, prompt, systemPrompt, _source) {
    let lastError = null;
    ...
    throw new Error("All Gemini models are unavailable (quota/not-found). " +
        "Please add your own Gemini API key in Settings → API Keys.");
}
async function callAnthropic(apiKey, prompt, systemPrompt, _source) {
...
async function callOpenAI(apiKey, prompt, systemPrompt, _source) {
...
// ─── Ollama (local) ───────────────────────────────────────────────────────────
async function callOllama(baseUrl, model, prompt, systemPrompt) {
    const messages = [];
    if (systemPrompt)
        messages.push({ role: "system", content: systemPrompt });
    messages.push({ role: "user", content: prompt });
    ...
    for (const tryModel of modelsToTry) {
        ...
        const text = data.choices?.[0]?.message?.content || "";
        if (!text)
            throw new Error(`Ollama returned empty response for model: ${tryModel}`);
        const usage = data.usage || {};
...
async function callBedrockModel(accessKeyId, secretAccessKey, bearerToken, model, prompt, systemPrompt) {
    ...
    if (!bearerToken)
        throw new Error("Bedrock: no credentials provided");
...
async function callBedrock(accessKeyId, secretAccessKey, bearerToken, prompt, systemPrompt, _source) {
    let lastError = null;
    for (const model of BEDROCK_MODELS) {
        try {
            ...
            const isSkippable = err.message?.includes("Legacy") ||
