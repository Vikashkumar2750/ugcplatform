...
exports.updateRateLimitConfig = updateRateLimitConfig;
...
async function getAccountState(accountId) {
    ...
    if (data) {
        ...
        const configOverride = (data.config_override || {});
        state = {
...
// ─── Check rate limit ────────────────────────────────────────────────────────
async function checkRateLimit(accountId, recipientId) {
...
async function recordSend(accountId, recipientId) {
    const state = await getAccountState(accountId);
    const now = Date.now();
    state.hourlyCount++;
    state.dailyCount++;
...
// ─── Persist state to DB ─────────────────────────────────────────────────────
async function persistState(accountId, state) {
    await supabase_1.supabase.from("rate_limit_state").upsert({
...
function getTypingDelay(messageText) {
    const len = messageText.length;
    if (len < 150)
        return 500; // 0.5s for short messages
    if (len < 300)
        return 1000; // 1s for medium messages
    if (len < 500)
        return 1500; // 1.5s for longer messages
    return Math.min(2000, 1500 + Math.floor((len - 500) / 200) * 250); // Max 2s
}
// ─── Update account rate limit config ────────────────────────────────────────
// Called from admin/settings UI.
async function updateRateLimitConfig(accountId, config) {
