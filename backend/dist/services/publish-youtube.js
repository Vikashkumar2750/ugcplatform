...
async function publishYoutubeVideo(post, token) {
    ...
    if (contentType === "community" || contentType === "text" || contentType === "post") {
        throw new Error("YouTube community/text posts cannot be published via the API. " +
    ...
    if (contentType === "photo" || contentType === "image" || contentType === "carousel") {
        throw new Error("YouTube does not support image/photo posts via the API. " +
            "Only video/Shorts uploads are supported.");
    }
    const videoUrl = post.media_urls?.[0] || post.media_url;
    if (!videoUrl)
        throw new Error("No video URL provided for YouTube. Upload a video first.");
