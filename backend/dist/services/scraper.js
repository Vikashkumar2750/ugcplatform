...
exports.scrapeCompetitorFull = scrapeCompetitorFull;
exports.scrapeInstagramProfileFull = scrapeInstagramProfileFull;
exports.searchFacebookGroups = searchFacebookGroups;
exports.scrapeYouTubeChannel = scrapeYouTubeChannel;
exports.scrapeLinkedInProfile = scrapeLinkedInProfile;
exports.scrapeLinkedInCompany = scrapeLinkedInCompany;
exports.runApifyActor = runApifyActor;
const RAPID_API_KEY = process.env.RAPIDAPI_KEY || "";
const APIFY_TOKEN = process.env.APIFY_TOKEN || "";
// ─── Instagram — Multiple API sources with fallback ──────────────────────────
async function scrapeInstagramProfile(username, userKeys) {
...
async function scrapeCompetitorFull(username, userKeys) {
    ...
    if (APIFY_TOKEN) {
        ...
        if (profileResult.status === "fulfilled" && profileResult.value.length > 0) {
            ...
            console.warn(`[scraper] Apify profile scraper failed for @${username}: ${profileResult.status === "rejected" ? profileResult.reason : "empty"}`);
        }
        // Process posts data
        if (postsResult.status === "fulfilled" && postsResult.value.length > 0) {
            rawPosts = postsResult.value.map((item) => ({
    ...
    const enhancedPosts = rawPosts.map(p => ({
        id: p.id,
        type: p.type || "IMAGE",
        caption: p.caption || "",
...
// ── Source 1: instagram120 RapidAPI ──────────────────────────────────────────
async function scrapeViaInstagram120(username, userKeys) {
    ...
    if (!postsRes.ok)
        throw new Error(`instagram120 posts: ${postsRes.status}`);
    ...
    let profile;
    try {
        const profileRes = await fetch(`https://instagram120.p.rapidapi.com/api/instagram/profile?username=${username}`, { method: "GET", headers });
        if (profileRes.ok) {
...
// ── Source 2: instagram-scraper3 RapidAPI ─────────────────────────────────────
async function scrapeViaInstagramScraper3(username, userKeys) {
    const key = userKeys?.rapidapi || RAPID_API_KEY;
    if (!key)
        throw new Error("No RapidAPI key provided");
    const headers = {
    ...
    const profileRes = await fetch(`https://instagram-scraper3.p.rapidapi.com/user/info?username=${username}`, { headers });
    if (!profileRes.ok)
        throw new Error(`instagram-scraper3 profile: ${profileRes.status}`);
    const profileData = await profileRes.json();
    ...
    const postsRes = await fetch(`https://instagram-scraper3.p.rapidapi.com/user/posts?username=${username}&count=20`, { headers });
    let posts = [];
    if (postsRes.ok) {
        const postsData = await postsRes.json();
...
async function scrapeViaApifyBasic(username, userKeys) {
    ...
    const posts = (p.latestPosts || p.posts || []).map((post) => ({
...
async function scrapeInstagramProfileFull(username) {
...
// ─── Facebook ─────────────────────────────────────────────────────────────────
async function searchFacebookGroups(query) {
    if (!RAPID_API_KEY)
        return { posts: [] };
    const res = await fetch(`https://facebook-scraper3.p.rapidapi.com/search/groups?query=${encodeURIComponent(query)}`, {
        headers: {
    ...
    if (!res.ok)
        throw new Error(`Facebook scrape failed: ${res.status}`);
...
async function scrapeYouTubeChannel(channelId) {
    ...
    if (!res.ok)
        throw new Error(`YouTube scrape failed: ${res.status}`);
    const data = await res.json();
...
async function scrapeLinkedInProfile(username) {
    if (!RAPID_API_KEY)
        return { posts: [], profile: { username } };
    try {
        const res = await fetch(`https://linkedin-data-api.p.rapidapi.com/get-profile-data-by-url?url=https://www.linkedin.com/in/${username}/`, {
            headers: {
        ...
        if (!res.ok)
            throw new Error(`LinkedIn profile: ${res.status}`);
        const data = await res.json();
        const profile = {
...
async function scrapeLinkedInCompany(domain) {
    if (!RAPID_API_KEY)
        return { posts: [] };
    const res = await fetch(`https://linkedin-data-api.p.rapidapi.com/get-company-by-domain?domain=${encodeURIComponent(domain)}`, {
        headers: {
            "x-rapidapi-host": "linkedin-data-api.p.rapidapi.com",
            "x-rapidapi-key": RAPID_API_KEY,
        },
    ...
    if (!res.ok)
        throw new Error(`LinkedIn scrape failed: ${res.status}`);
    const data = await res.json();
...
// ─── Apify ────────────────────────────────────────────────────────────────────
async function runApifyActor(actorId, input, userKeys) {
    ...
    let startData;
    try {
        const startRes = await fetch(`https://api.apify.com/v2/acts/${encodeURIComponent(actorId)}/runs?token=${token}`, {
            method: "POST",
        ...
        if (err.name === "AbortError")
            throw new Error(`Apify actor ${actorId} start timed out after 25s`);
        throw err;
    }
    ...
    const resultsRes = await fetch(`https://api.apify.com/v2/actor-runs/${runId}/dataset/items?token=${APIFY_TOKEN}&limit=50`);
    if (!resultsRes.ok)
        throw new Error(`Apify results fetch failed: ${resultsRes.status}`);
