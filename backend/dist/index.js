...
require("dotenv/config"); // load .env file for local dev (no-op in production)
...
const send_queue_1 = require("./services/send-queue");
...
node_cron_1.default.schedule("0 */6 * * *", async () => {
    const count = await refreshExpiringTokens();
    if (count > 0)
        console.log(`[${new Date().toISOString()}] Refreshed ${count} tokens`);
});
// ─────────────────────────────────────────────
...
node_cron_1.default.schedule("* * * * *", async () => {
    try {
        await (0, send_queue_1.recoverStaleMessages)();
    }
    catch (err) {
...
node_cron_1.default.schedule("0 * * * *", async () => {
    try {
        const cleaned = await (0, rate_limiter_1.cleanupExpiredRateLimits)();
        if (cleaned > 0)
            console.log(`[${new Date().toISOString()}] RateLimiter: cleaned ${cleaned} expired windows`);
    }
    catch (err) {
...
async function publishPost(post) {
    ...
    switch (post.platform) {
        ...
        case "youtube":
            return (0, publish_youtube_1.publishYoutubeVideo)(post, token);
        case "linkedin":
            return (0, publish_linkedin_1.publishLinkedinPost)(post, token);
        default:
            throw new Error(`Unsupported platform: ${post.platform}`);
    }
}
async function publishInstagramPost(post, token) {
...
async function waitForVideoUpload(containerId, token, maxAttempts = 12) {
    for (let i = 0; i < maxAttempts; i++) {
        ...
        if (status.status_code === "ERROR")
            throw new Error("Video upload failed on Instagram servers");
    }
...
async function refreshExpiringTokens() {
    ...
    for (const account of accounts) {
        try {
            const res = await fetch(`https://graph.instagram.com/refresh_access_token?grant_type=ig_refresh_token&access_token=${account.access_token}`);
            const data = await res.json();
...
// ─────────────────────────────────────────────
async function processUnhandledWebhookEvents() {
    ...
    for (const event of events) {
        try {
            ...
            else if (event.event_type === "instagram_message" || event.event_type === "facebook_message") {
                await handleMessageDMTrigger(event);
            }
...
async function handleCommentDMTrigger(event) {
    ...
    for (const rule of rules) {
        ...
        const keywords = (rule.trigger_config?.keywords || []).map((k) => k.toLowerCase());
        const matchType = rule.trigger_config?.match_type || "any";
        ...
        try {
            ...
            if (actions.dm) {
                if (rule.action_config?.require_follow) {
                    dmText = pickRandom(rule.action_config?.follow_prompt_messages) || "Please follow me and reply 'DONE' to get the link!";
                }
                else {
                    dmText = pickRandom(rule.action_config?.messages) || rule.action_config?.message || "Namaste! 🙏";
                }
            ...
            const replyText = pickRandom(rule.action_config?.reply_texts) || rule.action_config?.reply_text || "Check your DM!";
            let dmResult = null;
            let replyResult = null;
            if (actions.dm && dmText) {
                dmResult = await (0, send_queue_2.enqueueMessage)({
                ...
                // Schedule Follow-up DM if enabled and no follow-requirement
                if (!rule.action_config?.require_follow && rule.action_config?.follow_up_enabled && rule.action_config?.follow_up_delay > 0) {
                    ...
                    await (0, send_queue_2.enqueueMessage)({
                        accountId: account.id,
                        userId: account.user_id,
                        recipientId: senderId, // Note: Follow up needs to go to senderId, but standard 24h window must be open!
            ...
            if (actions.reply) {
                replyResult = await (0, send_queue_2.enqueueMessage)({
                    accountId: account.id,
                    userId: account.user_id,
