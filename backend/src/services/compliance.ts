...
export interface ComplianceResult {
  ...
  reasonDetail?: string;
  windowExpiresAt?: string;   // ISO8601 — when 24h window closes
}
...
export async function checkCompliance(input: ComplianceCheckInput): Promise<ComplianceResult> {
...
async function logComplianceDecision(
...
export async function isWithinMessagingWindow(
  accountId: string,
  recipientId: string
): Promise<{ inWindow: boolean; expiresAt?: string }> {
