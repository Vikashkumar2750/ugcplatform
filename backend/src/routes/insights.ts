...
function generateRuleBasedInsights(platform: string, data: any) {
  ...
  if (platform === "instagram") {
    if (er >= 6) engagementScore = 90;
    else if (er >= 3) engagementScore = 75;
    else if (er >= 1.5) engagementScore = 55;
    else engagementScore = 35;
  } else if (platform === "facebook") {
    if (er >= 3) engagementScore = 85;
    else if (er >= 1.5) engagementScore = 65;
    ...
    if (viewToSubRatio >= 0.2) engagementScore = 90;
    else if (viewToSubRatio >= 0.08) engagementScore = 75;
    else engagementScore = 45;
  }

  // Consistency Score
  let consistencyScore = 40;
  if (platform === "youtube") {
    if (posts30dCount >= 4) consistencyScore = 85;
    else if (posts30dCount >= 2) consistencyScore = 65;
    else consistencyScore = 45;
  } else {
    if (posts30dCount >= 12) consistencyScore = 90;
    else if (posts30dCount >= 6) consistencyScore = 70;
    else consistencyScore = 45;
  }
  ...
  if (reachPct !== undefined && reachPct !== null) {
    if (reachPct >= 20) growthScore = 85;
    else if (reachPct >= 0) growthScore = 70;
    else if (reachPct >= -15) growthScore = 50;
    else growthScore = 35;
  }
  ...
  if (platform === "instagram") {
    executiveSummary = `Your account is showing a healthy engagement rate of ${er.toFixed(1)}%. ${
      reachPct && reachPct < 0 
    ...
    executiveSummary = `Your Facebook Page has an engagement rate of ${er.toFixed(1)}%. ${
      mediaCount < 3 ? "Consistency needs improvement. Aim to post at least 3 times a week." : "Keep up the consistent scheduling to boost organic page reach."
    }`;
  } else {
  ...
  const topPostsAnalysis = (data.topPosts || []).map((p: any) => ({
  ...
  const underperformingPostsAnalysis = (data.topPosts || []).slice(-2).map((p: any) => ({
    postId: p.postId || p.id,
