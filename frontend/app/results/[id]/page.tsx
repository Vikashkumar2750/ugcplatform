...
function CompetitorsTab({ competitors: raw }: { competitors: any }) {
  try {
    ...
    const scrapedStats: any[] = isWrapped ? (raw.scrapedStats || []) : [];

    // Detect when LLM JSON parsing failed (extractJSON returned { raw: "..." })
    const hasAIData = competitors && !competitors.raw &&
      (competitors.detectedNiche || (competitors.competitors?.length ?? 0) > 0 || (competitors.keyInsights?.length ?? 0) > 0);
    ...
    return (<>
      ...
      {/* Scraped stats cards — always shown when available, even if AI JSON failed */}
            ...
            {scrapedStats.map((stat: any, i: number) => (
              <div key={i} className="flex items-center gap-4 p-3 rounded-xl border border-border">
                ...
                <div className="flex-1">
                  ...
                  {stat.bio && <p className="text-xs text-muted-foreground mt-0.5 line-clamp-1">{stat.bio}</p>}
                </div>
                <div className="text-right text-xs text-muted-foreground space-y-0.5">
                  {stat.followers != null && <p>{stat.followers?.toLocaleString() || "—"} followers</p>}
                  {stat.engagementRate != null && <p className="text-amber-500">{stat.engagementRate}% ER</p>}
                  {stat.totalPostsAnalyzed != null && <p>{stat.totalPostsAnalyzed} posts analyzed</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      {/* If AI JSON parsing failed, show a helpful re-run prompt */}
      {!hasAIData && (
        <div className="p-6 rounded-2xl border border-amber-400/20 bg-amber-400/5 text-center space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Data scraped successfully ({totalPostsScraped || 0} posts) but the AI response was cut short.
            Use “Re-analyze Competitors” above to regenerate the full analysis.
          </p>
        </div>
      )}
