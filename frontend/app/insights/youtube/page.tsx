...
function ScoreCard({ label, score, desc, icon: Icon }: { label: string; score: number; desc: string; icon: any }) {
  ...
  return (
    <div className="p-4 rounded-3xl border bg-card/40 flex flex-col items-center justify-center text-center gap-4 hover:shadow-md hover:bg-card transition-all border-border/40 hover:border-red-500/30 overflow-hidden w-full">
      <div className="relative flex-shrink-0 w-16 h-16 flex items-center justify-center">
        <svg className="w-full h-full transform -rotate-90">
          ...
          <circle cx="32" cy="32" r={radius} className="text-red-500 transition-all duration-1000 ease-out" stroke="currentColor" strokeWidth="4.5" fill="transparent"
...
export default function YouTubeInsightsPage() {
  ...
  const fetchAiData = useCallback(async (insJson: any) => {
    if (!insJson || !insJson.subscribers) return;
    setAiLoading(true);
    try {
      const res = await fetch("/api/insights/generate-ai", {
        ...
        body: JSON.stringify({ 
          platform: "youtube", 
          handle: insJson.channelName, 
          name: insJson.channelName, 
          statsData: insJson 
        })
      });
      if (res.ok) {
  ...
  if (loading) return (
    <div className="p-6 max-w-5xl mx-auto flex items-center justify-center min-h-[60vh]">
      <div className="text-center space-y-3">
        ...
        <p className="text-xs text-muted-foreground/60">Retrieving YouTube metrics and building dashboard</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto pb-16 space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3.5">
        ...
        <ScoreCard label="Growth Speed" score={scores.growthScore} desc="Subscribers & views growth rate" icon={TrendingUp} />
        <ScoreCard label="Engagement" score={scores.engagementScore} desc="Likes & comments ratios" icon={Heart} />
        <ScoreCard label="Content Quality" score={scores.contentScore} desc="Watch yield metrics" icon={Star} />
        <ScoreCard label="Consistency" score={scores.consistencyScore} desc="Video upload frequency logs" icon={Camera} />
      </div>
            ...
            <p className="text-sm leading-relaxed text-foreground/90 font-medium min-h-[40px]">
              {aiLoading ? (
                <span className="animate-pulse flex items-center gap-2 text-muted-foreground">
                  <RefreshCw className="w-4 h-4 animate-spin text-red-500" /> Generating insights...
                </span>
              ) : (
                ai.executiveSummary
              )}
              ...
              <div className="divide-y divide-border/50">
                {data.recentVideos.length > 0 ? (
                  data.recentVideos.slice(0, 5).map((video, i) => {
                    const aiAnalysis = ai.topPostsAnalysis?.find((a: any) => a.postId === video.id);
                    return (
                      <div key={video.id} className="p-4 flex flex-col hover:bg-muted/10 transition">
                        ...
                        <div className="bg-red-500/[0.02] border border-red-500/10 p-4 rounded-xl flex items-start gap-3 ml-9 mt-4">
                          <Sparkles className="w-4 h-4 text-red-500 flex-shrink-0 mt-0.5" />
                          <div className="text-xs text-red-700 dark:text-red-400 leading-relaxed font-medium space-y-2 w-full">
                            {aiLoading ? (
                              <span className="animate-pulse">Analyzing post anatomy...</span>
                            ) : aiAnalysis ? (
                              <div className="space-y-3">
                                {aiAnalysis.hookAnalysis && <div><strong className="text-red-500">Hook:</strong> {aiAnalysis.hookAnalysis}</div>}
                              ...
                              <p><strong>AI Success Factor:</strong> Strong visual hook and high initial CTR within the first 2 hours of upload.</p>
                            )}
                          </div>
                        </div>
                      </div>
